...
jobs:
  claude-response:
    ...
    steps:
      - uses: Trans-ltd/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: Trans-ltd/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`Trans-ltd/claude-code-base-action`](https://github.com/Trans-ltd/claude-code-base-action).
